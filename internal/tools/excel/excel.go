	...
	return &tools.ExtendedHelp{
		Examples: []tools.ToolExample{
			...
			},
			{
				...
				ExpectedResult: "Returns all sheets with data in CSV format, perfect for AI agent ingestion. Each sheet includes sheet_name, data (as CSV string), and dimensions.",
			},
			{
		...
		ParameterDetails: map[string]string{
			...
			"read_all_data.sheet_names":      "Array of specific sheet names to read (e.g., ['Sales', 'Expenses']). If omitted, reads all sheets. Use get_workbook_metadata to discover sheet names first.",
		},
