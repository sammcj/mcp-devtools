...
func (t *GraphvizDiagramTool) renderDiagram(ctx context.Context, logger *logrus.Logger, dotContent string, diagram *DiagramSpec, outputFormats []string, filename, workspaceDir string) (map[string]interface{}, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("failed to determine output directory (workspaceDir='%s'): %w. Try specifying an absolute workspace_dir parameter", workspaceDir, err)
	}
	...
	for _, formatStr := range outputFormats {
		...
		if formatStr == "dot" {
			...
			if err := os.WriteFile(outputPath, buf.Bytes(), 0600); err != nil {
				return nil, fmt.Errorf("failed to write %s file: %w", formatStr, err)
			}
		}
...
func (t *GraphvizDiagramTool) getOutputDirectory(workspaceDir string) (string, error) {
	var baseDir string

	// Use provided workspace directory or current working directory
	if workspaceDir != "" {
		// Validate the provided workspace directory
		if !filepath.IsAbs(workspaceDir) {
			return "", fmt.Errorf("workspace_dir must be an absolute path, got: %s", workspaceDir)
		}
	...
	// Validate baseDir exists and is accessible
	if stat, err := os.Stat(baseDir); err != nil {
		return "", fmt.Errorf("base directory '%s' is not accessible: %w", baseDir, err)
	} else if !stat.IsDir() {
		return "", fmt.Errorf("base directory '%s' is not a directory", baseDir)
	}
...
func (t *GraphvizDiagramTool) generateFilename(diagramName string) string {
...
func (t *GraphvizDiagramTool) writeDOTFile(outputPath, dotContent string) error {
