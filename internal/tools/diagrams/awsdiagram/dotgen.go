...
func (t *AWSDiagramTool) generateDOT(diagram *DiagramSpec) (string, error) {
	...
	dot.WriteString("  node [shape=box, style=rounded, fontname=\"Arial\", fontsize=12];\n")
...
func (t *AWSDiagramTool) generateCluster(dot *strings.Builder, cluster ClusterSpec, index int, diagram *DiagramSpec) error {
	// Start subgraph
...
func (t *AWSDiagramTool) generateNode(dot *strings.Builder, node NodeSpec) error {
	return t.generateNodeWithIndent(dot, node, "  ")
}

// generateNodeWithIndent generates DOT for a node with specified indentation
func (t *AWSDiagramTool) generateNodeWithIndent(dot *strings.Builder, node NodeSpec, indent string) error {
...
func (t *AWSDiagramTool) generateConnection(dot *strings.Builder, conn ConnectionSpec) error {
	// Basic connection
...
func (t *AWSDiagramTool) getNodeStyling(nodeType string) (shape, color, style string) {
	// Default styling
	shape = "box"
	color = "lightblue"
