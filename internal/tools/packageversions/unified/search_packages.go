...
func (t *SearchPackagesTool) Definition() mcp.Tool {
	return mcp.NewTool(
		"search_packages",
		mcp.WithDescription("Search for packages and check versions across multiple ecosystems (npm, Go, Python, Java, Swift, GitHub Actions, Docker, AWS Bedrock). This unified tool replaces individual package manager tools and provides a single interface for package discovery and version checking."),
		...
		mcp.WithString("query",
			mcp.Description("The search query. For most ecosystems, this should be a package name or dependency object. For bedrock, use model search terms. For docker, use image names."),
			mcp.Required(),
		),
		mcp.WithObject("data",
			mcp.Description("Ecosystem-specific data object. For npm: dependencies object from package.json. For go: dependencies from go.mod. For python: requirements array. For java: dependencies array. For swift: dependencies array. For github-actions: actions array. For docker: image query object. For bedrock: optional filters."),
		),
		mcp.WithObject("constraints",
			mcp.Description("Optional constraints for specific packages (version constraints, exclusions, etc.)"),
		),
		mcp.WithString("action",
			mcp.Description("Optional action for specific ecosystems. For bedrock: 'list', 'search', 'get'. For docker: 'tags', 'info'. Defaults to appropriate action for ecosystem."),
			mcp.Enum("list", "search", "get", "tags", "info"),
		),
		mcp.WithNumber("limit",
			mcp.Description("Maximum number of results to return (where applicable)"),
		),
		mcp.WithString("registry",
			mcp.Description("Specific registry to use (for docker: 'dockerhub', 'ghcr', 'custom')"),
		),
		mcp.WithBoolean("includeDetails",
			mcp.Description("Include additional details in results (where applicable)"),
		),
	)
}
