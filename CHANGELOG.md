...
- Browser-flow discovery rejects a metadata document with no `issuer`, which was previously read as "nothing to compare" and had its endpoints applied unbound. It also refuses redirects off the configured origin and caps the response at 256KB, matching the proxy's discovery.
